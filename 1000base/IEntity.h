...
public:
	...
	{
		return *(C_BasePlayerAnimState**)(PVOID)((DWORD)(this) + 0x38A0);
	}

	void SetAnimState(C_BasePlayerAnimState* State)
	{
		*(C_BasePlayerAnimState**)(PVOID)((DWORD)(this) + 0x38A0) = State;
	}
	...
	void setAbsOriginal(Vector origin)
	{
		...
		if (!SetAbsOrigin)
			SetAbsOrigin = (SetAbsOriginFn)(g_Utils->FindPattern("client.dll", (PBYTE)"\x55\x8B\xEC\x83\xE4\xF8\x51\x53\x56\x57\x8B\xF1\xE8\x00\x00", "xxxxxxxxxxxxx??") + 0x2);
	...
	Vector GetViewPunch()
	{
		return *reinterpret_cast<Vector*>((DWORD)this + 0x3010);
	}
	Vector* GetPunch()
	{
		return reinterpret_cast<Vector*>((DWORD)this + 0x301C);
	...
	bool IsScoped()
	{
	//	return *(bool *)((DWORD)this + 0x38AA);
	//	return *reinterpret_cast<bool*>(uintptr_t(this) + uintptr_t(0x38AA));
		//static int m_bIsScoped = g_pNetvars->GetOffset("DT_CSPlayer", "m_bIsScoped");
		//return GetValue<bool>(m_bIsScoped);
		return *reinterpret_cast<bool*>((DWORD)this + g_pNetvars->GetOffset("DT_CSPlayer", "m_bIsScoped"));
	}
	...
	{
		static int m_fFlags = g_pNetvars->GetOffset("DT_BasePlayer", "m_fFlags");
		*reinterpret_cast<int*>(uintptr_t(this) + m_fFlags) = flags;
	}
	...
	Vector GetHitboxPosition2(int Hitbox, matrix3x4_t matrix[]);
	Vector GetHitboxPosition(int Hitbox, matrix3x4_t matrix[]);

	Vector GetHitboxPosition3(int Hitbox, matrix3x4_t matrix[], Vector * vMin, Vector * vMax);
...
//	void setup_bone_matrix(C_BaseEntity * entity, matrix3x4_t * mat);
	std::vector<Vector> GetMultiplePointsForHitbox(int iHitbox, matrix3x4_t matrix[]);

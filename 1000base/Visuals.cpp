...
void Visuals::SetupDraw() {
	...
	/*
	if (g_Globals->LocalPlayer->IsScoped() && g_Globals->LocalPlayer->GetHealth() > 0 && g_Vars->Aimbot.Enable)
	{
		int iScreenWidth, iScreenHeight;
		I::Engine->GetScreenSize(iScreenWidth, iScreenHeight);
		I::Surface->DrawSetColor(Color::Green());
		I::Surface->DrawOutlinedRect(iScreenWidth / 2, 0, iScreenWidth / 2, iScreenHeight);
		I::Surface->DrawOutlinedRect(0, iScreenHeight / 2, iScreenWidth, iScreenHeight / 2);
	}
	*/
...
void Visuals::RenderPlayers(C_BaseEntity* entity, float alpha, matrix3x4_t* bone_matrix)
{
	...
	if (g_Utils->WorldToScreen(pos3D, pos) && g_Utils->WorldToScreen(top3D, top))
	{
		...
		Vector posscan;
		Vector hitscan;
		if (g_Utils->WorldToScreen(g_Globals->scanvector, posscan) && g_Utils->WorldToScreen(g_Globals->LastAimVec, hitscan))
		{
			I::Surface->DrawSetColor(Color::Red());
			I::Surface->DrawOutlinedCircle(posscan.x, posscan.y, 15, 30);
			I::Surface->DrawSetColor(Color::Blue());
			I::Surface->DrawOutlinedCircle(hitscan.x, hitscan.y, 10, 30);
			I::Surface->DrawT(posscan.x, posscan.y, Color(255, 255, 255, 255), g_Globals->Espfont, true, StringUpper(std::to_string(g_Globals->damage)).c_str());
		}
		//--- Player Box ---//
		if (g_Vars->Visuals.Box)
		{
			I::Surface->DrawSetColor(Color(255, 254, 255, alpha));
		...
		if (g_Vars->Visuals.Ammo && !entity->IsKnifeorNade())
		{
			...
			{
				float cycle = animLayer->m_flCycle; // 1 = finished 0 = just started //cfbei
		...
		//--- Player Bones ---//
			if (g_Vars->Visuals.Skeleton)
			{
				...
				static Vector plhead;
				static Vector plneck;
				static Vector plpelvis;

			

				for (int i = 0; i < 19; i++)
				{
					g_Utils->WorldToScreen(g_Globals->LocalPlayer->GetHitboxPosition(i, g_Globals->FakeMatrix), HitboxW2S[i]);

				}
				/*
				I::Surface->DrawSetColor(Color(255, 255, 255, alpha));
			
				if (g_Autowall->CanHit(g_Aimbot->pTarget->GetEyePosition(), g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_HEAD, g_Globals->RealMatrix)))
				{
					Vector screen;
					Vector cross = g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_HEAD, g_Globals->RealMatrix);
					static int Scale = 2;
					if (g_Utils->WorldToScreen(cross, screen))
					{
						I::Surface->DrawFilledRect(screen.x - Scale, screen.y - (Scale * 2), (Scale * 2), (Scale * 4));
						I::Surface->DrawFilledRect(screen.x - (Scale * 2), screen.y - Scale, (Scale * 4), (Scale * 2));
						I::Surface->DrawFilledRect(screen.x - Scale - 1, screen.y - (Scale * 2) - 1, (Scale * 2) - 2, (Scale * 4) - 2);
						I::Surface->DrawFilledRect(screen.x - (Scale * 2) - 1, screen.y - Scale - 1, (Scale * 4) - 2, (Scale * 2) - 2);
					}
				}
				if (g_Autowall->CanHit(g_Aimbot->pTarget->GetEyePosition(), g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_NECK, g_Globals->RealMatrix)))
				{
					Vector screen;
					Vector cross = g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_NECK, g_Globals->RealMatrix);
					static int Scale = 2;
					if (g_Utils->WorldToScreen(cross, screen))
					{
						I::Surface->DrawFilledRect(screen.x - Scale, screen.y - (Scale * 2), (Scale * 2), (Scale * 4));
				...
				if (g_Autowall->CanHit(g_Aimbot->pTarget->GetEyePosition(), g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_PELVIS, g_Globals->RealMatrix)))
				{
					Vector screen;
					Vector cross = g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_PELVIS, g_Globals->RealMatrix);
					static int Scale = 2;
					if (g_Utils->WorldToScreen(cross, screen))
					{
						I::Surface->DrawFilledRect(screen.x - Scale, screen.y - (Scale * 2), (Scale * 2), (Scale * 4));
				...
				if (g_Autowall->CanHit(g_Aimbot->pTarget->GetEyePosition(), g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_BELLY, g_Globals->RealMatrix)))
				{
					Vector screen;
					Vector cross = g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_BELLY, g_Globals->RealMatrix);
					static int Scale = 2;
					if (g_Utils->WorldToScreen(cross, screen))
					{
						I::Surface->DrawFilledRect(screen.x - Scale, screen.y - (Scale * 2), (Scale * 2), (Scale * 4));
				...
				if (g_Autowall->CanHit(g_Aimbot->pTarget->GetEyePosition(), g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_THORAX, g_Globals->RealMatrix)))
				{
					Vector screen;
					Vector cross = g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_THORAX, g_Globals->RealMatrix);
					static int Scale = 2;
					if (g_Utils->WorldToScreen(cross, screen))
					{
						I::Surface->DrawFilledRect(screen.x - Scale, screen.y - (Scale * 2), (Scale * 2), (Scale * 4));
				...
				if (g_Autowall->CanHit(g_Aimbot->pTarget->GetEyePosition(), g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_UPPER_CHEST, g_Globals->RealMatrix)))
				{
					Vector screen;
					Vector cross = g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_UPPER_CHEST, g_Globals->RealMatrix);
					static int Scale = 2;
					if (g_Utils->WorldToScreen(cross, screen))
					{
						I::Surface->DrawFilledRect(screen.x - Scale, screen.y - (Scale * 2), (Scale * 2), (Scale * 4));
				...
				if (g_Autowall->CanHit(g_Aimbot->pTarget->GetEyePosition(), g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_LOWER_CHEST, g_Globals->RealMatrix)))
				{
					Vector screen;
					Vector cross = g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_LOWER_CHEST, g_Globals->RealMatrix);
					static int Scale = 2;
					if (g_Utils->WorldToScreen(cross, screen))
					{
						I::Surface->DrawFilledRect(screen.x - Scale, screen.y - (Scale * 2), (Scale * 2), (Scale * 4));
				...
				if (g_Autowall->CanHit(g_Aimbot->pTarget->GetEyePosition(), g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_RIGHT_THIGH, g_Globals->RealMatrix)))
				{
					Vector screen;
					Vector cross = g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_RIGHT_THIGH, g_Globals->RealMatrix);
					static int Scale = 2;
					if (g_Utils->WorldToScreen(cross, screen))
					{
						I::Surface->DrawFilledRect(screen.x - Scale, screen.y - (Scale * 2), (Scale * 2), (Scale * 4));
				...
				if (g_Autowall->CanHit(g_Aimbot->pTarget->GetEyePosition(), g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_LEFT_THIGH, g_Globals->RealMatrix)))
				{
					Vector screen;
					Vector cross = g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_LEFT_THIGH, g_Globals->RealMatrix);
					static int Scale = 2;
					if (g_Utils->WorldToScreen(cross, screen))
					{
						I::Surface->DrawFilledRect(screen.x - Scale, screen.y - (Scale * 2), (Scale * 2), (Scale * 4));
				...
				if (g_Autowall->CanHit(g_Aimbot->pTarget->GetEyePosition(), g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_RIGHT_CALF, g_Globals->RealMatrix)))
				{
					Vector screen;
					Vector cross = g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_RIGHT_CALF, g_Globals->RealMatrix);
					static int Scale = 2;
					if (g_Utils->WorldToScreen(cross, screen))
					{
						I::Surface->DrawFilledRect(screen.x - Scale, screen.y - (Scale * 2), (Scale * 2), (Scale * 4));
				...
				if (g_Autowall->CanHit(g_Aimbot->pTarget->GetEyePosition(), g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_RIGHT_FOOT, g_Globals->RealMatrix)))
				{
					Vector screen;
					Vector cross = g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_RIGHT_FOOT, g_Globals->RealMatrix);
					static int Scale = 2;
					if (g_Utils->WorldToScreen(cross, screen))
					{
						I::Surface->DrawFilledRect(screen.x - Scale, screen.y - (Scale * 2), (Scale * 2), (Scale * 4));
				...
				if (g_Autowall->CanHit(g_Aimbot->pTarget->GetEyePosition(), g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_LEFT_FOOT, g_Globals->RealMatrix)))
				{
					Vector screen;
					Vector cross = g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_LEFT_FOOT, g_Globals->RealMatrix);
					static int Scale = 2;
					if (g_Utils->WorldToScreen(cross, screen))
					{
						I::Surface->DrawFilledRect(screen.x - Scale, screen.y - (Scale * 2), (Scale * 2), (Scale * 4));
				...
				if (g_Autowall->CanHit(g_Aimbot->pTarget->GetEyePosition(), g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_RIGHT_HAND, g_Globals->RealMatrix)))
				{
					Vector screen;
					Vector cross = g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_RIGHT_HAND, g_Globals->RealMatrix);
					static int Scale = 2;
					if (g_Utils->WorldToScreen(cross, screen))
					{
						I::Surface->DrawFilledRect(screen.x - Scale, screen.y - (Scale * 2), (Scale * 2), (Scale * 4));
				...
				if (g_Autowall->CanHit(g_Aimbot->pTarget->GetEyePosition(), g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_LEFT_HAND, g_Globals->RealMatrix)))
				{
					Vector screen;
					Vector cross = g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_LEFT_HAND, g_Globals->RealMatrix);
					static int Scale = 2;
					if (g_Utils->WorldToScreen(cross, screen))
					{
						I::Surface->DrawFilledRect(screen.x - Scale, screen.y - (Scale * 2), (Scale * 2), (Scale * 4));
				...
				if (g_Autowall->CanHit(g_Aimbot->pTarget->GetEyePosition(), g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_RIGHT_UPPER_ARM, g_Globals->RealMatrix)))
				{
					Vector screen;
					Vector cross = g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_UPPER_CHEST, g_Globals->RealMatrix);
					static int Scale = 2;
				...
				if (g_Autowall->CanHit(g_Aimbot->pTarget->GetEyePosition(), g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_LEFT_UPPER_ARM, g_Globals->RealMatrix)))
				{
					Vector screen;
					Vector cross = g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_LEFT_UPPER_ARM, g_Globals->RealMatrix);
					static int Scale = 2;
					if (g_Utils->WorldToScreen(cross, screen))
					{
						I::Surface->DrawFilledRect(screen.x - Scale, screen.y - (Scale * 2), (Scale * 2), (Scale * 4));
				...
				if (g_Autowall->CanHit(g_Aimbot->pTarget->GetEyePosition(), g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_RIGHT_FOREARM, g_Globals->RealMatrix)))
				{
					Vector screen;
					Vector cross = g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_RIGHT_FOREARM, g_Globals->RealMatrix);
					static int Scale = 2;
					if (g_Utils->WorldToScreen(cross, screen))
					{
						I::Surface->DrawFilledRect(screen.x - Scale, screen.y - (Scale * 2), (Scale * 2), (Scale * 4));
				...
				if (g_Autowall->CanHit(g_Aimbot->pTarget->GetEyePosition(), g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_LEFT_FOREARM, g_Globals->RealMatrix)))
				{
					Vector screen;
					Vector cross = g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_LEFT_FOREARM, g_Globals->RealMatrix);
					static int Scale = 2;
					if (g_Utils->WorldToScreen(cross, screen))
					{
						I::Surface->DrawFilledRect(screen.x - Scale, screen.y - (Scale * 2), (Scale * 2), (Scale * 4));
				...
				/*
				if (g_Autowall->CanHit(g_Aimbot->pTarget->GetEyePosition(), g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_MAX, g_Globals->RealMatrix)))
				{
					Vector screen;
					Vector cross = g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_MAX, g_Globals->RealMatrix);
					static int Scale = 2;
					if (g_Utils->WorldToScreen(cross, screen))
					{
						I::Surface->DrawFilledRect(screen.x - Scale, screen.y - (Scale * 2), (Scale * 2), (Scale * 4));
				...
				g_Utils->WorldToScreen(g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_NECK, g_Globals->FakeMatrix), plneck);
				g_Utils->WorldToScreen(g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_PELVIS, g_Globals->FakeMatrix), plpelvis);
				g_Utils->WorldToScreen(g_Globals->LocalPlayer->GetHitboxPosition(HITBOX_HEAD, g_Globals->FakeMatrix), plhead);
				if (g_Globals->RealAngle.y != g_Globals->LocalPlayer->GetLowerBodyYaw())
					color = Color(Color::Green().r(), Color::Green().g(), Color::Green().b(), alpha);
				else
					color = Color(Color::Red().r(), Color::Red().g(), Color::Red().b(), alpha);
				...
				studiohdr_t *studioHdr = I::ModelInfo->GetStudiomodel(g_Globals->LocalPlayer->GetModel());
				if (studioHdr) {
					for (int i = 0; i < studioHdr->numbones; i++)
					{
						...
						Vector bonePos1;
						if (!g_Utils->WorldToScreen(Vector(g_Globals->FakeMatrix[i][0][3], g_Globals->FakeMatrix[i][1][3], g_Globals->FakeMatrix[i][2][3]), bonePos1))
							continue;

						Vector bonePos2;
						if (!g_Utils->WorldToScreen(Vector(g_Globals->FakeMatrix[bone->parent][0][3], g_Globals->FakeMatrix[bone->parent][1][3], g_Globals->FakeMatrix[bone->parent][2][3]), bonePos2))
							continue;

						I::Surface->DrawSetColor(color);
						I::Surface->DrawLine((int)bonePos1.x, (int)bonePos1.y, (int)bonePos2.x, (int)bonePos2.y);

						I::Surface->DrawSetColor(Color(255, 0, 0, alpha));
						I::Surface->DrawOutlinedCircle(plhead.x, plhead.y, 16, 30);

						I::Surface->DrawFilledCircle(plhead, Color(255, 0, 134, alpha), 15, 30);

						I::Surface->DrawOutlinedRect(plneck.x + 7, plneck.y, plpelvis.x + 8, plpelvis.y + 3);
	...
	else
	{
		...
		I::Engine->GetScreenSize(screen_w, screen_h);
		I::Engine->GetViewAngles(eyeangles);
		Vector newangle = g_Math->CalcAngle(Vector(g_Globals->LocalPlayer->GetAbsOrigin().x, g_Globals->LocalPlayer->GetAbsOrigin().y, 0), Vector(entity->GetAbsOrigin().x, entity->GetAbsOrigin().y, 0));
		g_Math->AngleVectors(Vector(0, 270, 0) - newangle + Vector(0, eyeangles.y, 0), &poopvec);
		auto circlevec = Vector(screen_w / 2, screen_h / 2, 0) + (poopvec * 250.f);
		I::Surface->DrawFilledCircle(circlevec, entity->IsDormant() ? Color(100, 100, 100, 100) : Color(255, 0, 0, 100), 10, 30);

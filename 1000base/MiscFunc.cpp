...
void MiscFunc::AnimFix(ClientFrameStage_t curStage)
{
	if (I::Engine->IsInGame() && I::Engine->IsConnected())
	{
		for (int i = 1; i <= I::Engine->GetMaxClients(); ++i)
		{
			...
			auto& AnimState = *reinterpret_cast<uintptr_t*>(uintptr_t(Entity) + ptrdiff_t(0x38A0));
			if (!AnimState)
				continue;
...
void MiscFunc::ThirdPerson(ClientFrameStage_t curStage)
{
	if (I::Engine->IsInGame() && I::Engine->IsConnected())
	{
		if (g_Globals->LocalPlayer)
		{
			...
			if (GetAsyncKeyState(VK_MBUTTON) && g_Globals->LocalPlayer->IsAlive())
			{
				if (!check)
					enabledtp = !enabledtp;
				check = true;
				...
				check = false;
			if (g_Vars->Misc.ThirdPerson && enabledtp)
			{
				if (init)
				{
			...
			if (curStage == FRAME_RENDER_START && g_Vars->Misc.ThirdPerson && enabledtp &&  g_Globals->LocalPlayer->IsAlive() && (g_Vars->HvH.Yaws > 0 || g_Vars->HvH.Pitch > 0)) //&& !g_Globals->LocalPlayer->IsKnifeorNade()
			{

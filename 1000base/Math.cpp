...
std::vector<Vector> Math::GetMultiplePointsForHitbox(C_BaseEntity* local, C_BaseEntity* entity, int iHitbox, matrix3x4_t BoneMatrix[128])
{
	...
	std::vector<Vector> vecArray2;
	Vector max;
	Vector min;
	g_Math->VectorTransform(hitbox->min, BoneMatrix[hitbox->bone], max);
	g_Math->VectorTransform(hitbox->max, BoneMatrix[hitbox->bone], min);
	...
	case 0:
		...
		for (auto i = 0; i < 6; ++i)
		{
			...
			vecArray2.emplace_back(center);
		}
		vecArray[1] += Top * (hitbox->radius *  g_Vars->Aimbot.BodyPointscale);
		vecArray[2] += Right * (hitbox->radius *  g_Vars->Aimbot.BodyPointscale);
		vecArray[3] += Left * (hitbox->radius *  g_Vars->Aimbot.BodyPointscale);
		break;
	...
	if (entity->GetVelocity().Length2D() > 235.f )
		return vecArray2;
	else
	    return vecArray;
...
bool Math::skeetclamp(Vector &angles)
{
	Vector a = angles;
	NormalizeLemon(a);
	ClampAnglesLemon(a);

	...
	void ApplyChams(C_BaseEntity* ent)
	{
		if (g_Vars->Visuals.Chams) 
		{
			...
			for (int i = 1; i <= 64; ++i) 
			{
				...
				if (!ent || !g_Globals->LocalPlayer || !ent->IsAlive() || ent->IsDormant() || ent->IsGhost())
					continue;
				...
				float backtrack[4] = { .188f, .114f, .235f, 1.f };
				float visible[4] = { .588f, .784f, .235f, 1.f };
			
				
			
				I::RenderView->SetColorModulation(invisible);
					I::ModelRender->ForcedMaterialOverride(IgnorezTextured);
					ent->DrawModel(0x1, 255);
					I::ModelRender->ForcedMaterialOverride(nullptr);
				
				I::RenderView->SetColorModulation(visible);
				I::ModelRender->ForcedMaterialOverride(NotIgnorezTextured);
				ent->DrawModel(0x1, 255);
				I::ModelRender->ForcedMaterialOverride(nullptr);
				float cf[3];
